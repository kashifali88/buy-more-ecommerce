    ...
    <div>
      ...
      <div className="flex gap-3">
        ...
        <div onClick= {()=> setSizes(prev => prev.includes('S') ? prev.filter(item => item !== 'S') : [...prev,'S'] )}>
          <p className={`px-3 py-1 cursor-pointer  ${sizes.includes('S') ? "bg-black text-white" : "bg-slate-200"}`}>S</p>
        </div>
        
        <div onClick= {()=> setSizes(prev => prev.includes('M') ? prev.filter(item => item !== 'M') : [...prev, 'M'] )}>
          <p className={`px-3 py-1 cursor-pointer ${sizes.includes('M') ? "bg-black text-white" : "bg-slate-200"}`}>M</p>
        </div>

        <div onClick= {()=> setSizes(prev => prev.includes('L') ? prev.filter(item => item !== 'L') : [...prev, 'L'] )}> 
          <p className={`px-3 py-1 cursor-pointer ${sizes.includes('L') ? "bg-black text-white" : "bg-slate-200"}`}>L</p>
        </div>

        <div onClick= {()=> setSizes(prev => prev.includes('XL') ? prev.filter(item => item !== 'XL') : [...prev, 'XL'] )}>
          <p className={`px-3 py-1 cursor-pointer ${sizes.includes('XL') ? "bg-black text-white" : "bg-slate-200"}`}>XL</p>
        </div>

        <div onClick= {()=> setSizes(prev => prev.includes('XXL') ? prev.filter(item => item !== 'XXL') : [...prev, 'XXL'] )}>
          <p className={`px-3 py-1 cursor-pointer ${sizes.includes('XXL') ? "bg-black text-white" : "bg-slate-200"}`}>XXL</p>
        </div>
...
<button className="text-white bg-black w-20 py-3  rounded-md" type="submit">Add</button>
